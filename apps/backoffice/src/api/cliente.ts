 ...
 *
 * Comparte forma con el de la app de campo a propósito, pero es un fichero
 * distinto: allí `esFalloDeRed` decide si una operación se encola, y aquí solo
 * decide qué mensaje se enseña. Unificarlos obligaría a arrastrar la semántica
 * offline a una app que no la tiene.
 *
 * La clave del token también difiere: los dos front pueden convivir en el
 * mismo dominio y con el mismo navegador, y compartir clave haría que entrar
 * como supervisor cerrara la sesión del comercial.
 */

const CLAVE_TOKEN = "sw.bo.token";
